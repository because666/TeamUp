...
- CloudBase 技术验证使用 `npm run dev:mp-weixin:cloudbase` 或 `npm run build:mp-weixin:cloudbase`；环境 ID 和服务名仅从未提交的本地配置注入。
- API 模式必须配置包含 `/api/v1` 的 `VITE_API_BASE_URL`；非本地地址使用 HTTPS，微信 code 和平台 access token 只在 `src/services/` 边界处理。
- CloudBase 模式必须配置 `VITE_CLOUDBASE_ENV_ID` 和 `VITE_CLOUDBASE_SERVICE`，只能由 services 层调用 `wx.cloud.callContainer`；页面不得直接调用云托管或云数据库。

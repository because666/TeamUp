...
def make_app(settings: Settings | None = None, wechat_client: WeChatCodeExchanger | None = None) -> FastAPI:
    ...
    app = FastAPI(
        title="TeamUp API",
        version="0.1.0",
        description="TeamUp P0 backend foundation. Persistence is local/test memory only until the MySQL adapter is accepted.",
    ...
    async def get_profile(request: Request, user_id: str = Depends(current_user)):
    ...
    async def save_profile(payload: ProfileUpdate, request: Request, user_id: str = Depends(current_user)):
        profile = store.save_profile(user_id, ProfilePayload.model_validate(payload.model_dump(exclude={"version"})), payload.version)
        return envelope(request, profile.model_dump(mode="json"))

    @api.post("/projects", tags=["projects"], response_model=Envelope[ProjectData])
    async def create_project(payload: ProjectPayload, request: Request, user_id: str = Depends(current_user)):
        project = store.create_project(user_id, payload)
    ...
    async def get_project(project_id: str, request: Request, user_id: str = Depends(current_user)):
    ...
    async def update_project(project_id: str, payload: ProjectUpdate, request: Request, user_id: str = Depends(current_user)):
        project = store.update_project(user_id, project_id, payload)
        return envelope(request, project.model_dump(mode="json"))

    @api.post("/projects/{project_id}/publish", tags=["projects"], response_model=Envelope[ProjectData])
    async def publish_project(project_id: str, payload: ProjectAction, request: Request, user_id: str = Depends(current_user)):
        project = store.publish_project(user_id, project_id, payload.version)
        return envelope(request, project.model_dump(mode="json"))

    @api.post("/projects/{project_id}/close", tags=["projects"], response_model=Envelope[ProjectData])
    async def close_project(project_id: str, payload: ProjectAction, request: Request, user_id: str = Depends(current_user)):
        project = store.close_project(user_id, project_id, payload.version)
        return envelope(request, project.model_dump(mode="json"))
    ...
    async def list_projects(
        ...
        del user_id
        projects, next_cursor = store.list_projects(status, limit, cursor)
